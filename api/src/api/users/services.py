...
class UserService:
    ...
    async def get_paginated_users(self, username_contains: str | None = None, is_admin: bool | None = None, page: int = 1, size: int = 20) -> PagedResponse[UserModel]:
        return await self.user_repository.get_paginated_users(filters=UserRepository.FilterParams(username_contains=username_contains, is_admin=is_admin), page=page, size=size)
